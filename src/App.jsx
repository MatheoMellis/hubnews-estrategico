...
function App() {
  ...
  // Queries baseadas nas abas para a API GNews
  const getQueryForCategory = (main, sub) => {
    const queries = {
      ti: {
        all: 'concurso AND tecnologia',
        bancos: 'concurso AND (banco do brasil OR caixa economica)',
        federal: 'concurso AND (federal OR tribunal OR tse OR inss)',
        policial: 'concurso AND policia',
        universidades: 'concurso AND (universidade OR ufrj OR usp)'
      },
      ai: {
        all: 'inteligencia artificial',
        local: 'IA local OR edge ai OR gemma',
        cloud: 'openai OR gemini OR anthropic OR claude',
        agents: 'IA agentes OR automação autonomo'
      },
      news: {
        all: 'atualidades Brasil mundo',
        brasil: 'governo OR tecnologia Brasil',
        mundo: 'internacional tecnologia',
        economia: 'economia OR ibovespa OR mercado'
      }
    };
    return queries[main][sub] || 'tecnologia';
  };
  ...
  useEffect(() => {
    ...
    if (apiKey) {
      ...
      // GNews API (100 chamadas diárias gratuitas)
      const url = `https://gnews.io/api/v4/search?q=${encodeURIComponent(query)}&lang=pt&country=br&max=9&sortby=publishedAt&apikey=${apiKey}`;
      ...
      fetch(url)
        ...
        .catch(err => console.error("Erro ao buscar notícias API:", err))
        .finally(() => setIsLoading(false));
    } else {
      // Falha para dados estáticos se não houver chave
      setDynamicNews(NEWS_DATA.filter(item => {
        const matchMain = item.mainCat === activeMain;
        const matchSub = activeSub === 'all' || item.subCat === activeSub;
        return matchMain && matchSub;
      }));
    }
  }, [activeMain, activeSub, isAuthenticated, apiKey]);

  // Filtro extra de busca manual no input
      ...
      <div className="min-h-screen bg-[#000000] flex items-center justify-center p-4">
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button 
              type="submit"
              className="w-full bg-white text-black font-semibold rounded-lg py-3 text-sm hover:bg-[#E5E5E5] transition-colors mt-4"
            >
              Entrar
            </button>
          </form>
    ...
    <div className="h-screen w-full bg-[#000000] flex overflow-hidden relative">
        ...
        {showSettings && (
          ...
          >
             <div className="bg-[#0A0A0A] border border-[#262626] w-full max-w-md rounded-2xl p-6 shadow-2xl relative">
                ...
                <p className="text-[#A1A1AA] text-sm mb-6">Insira sua Chave de API da GNews para receber notícias reais diariamente. É 100% grátis.</p>
                ...
                <div className="mb-4">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#000000] border border-[#262626] rounded-md py-2.5 px-3 text-sm text-white focus:outline-none focus:border-white transition-colors"
                  />
                </div>
                <div className="flex gap-3 justify-end mt-8">
                  ...
                  <button onClick={saveApiKey} className="px-4 py-2 text-sm text-black bg-white rounded-md font-medium hover:bg-[#E5E5E5]">Salvar Configuração</button>
                </div>
             </div>
          </motion.div>
        )}
      ...
      <div className="md:hidden absolute top-0 left-0 right-0 h-14 border-b border-[#262626] bg-[#000000]/80 backdrop-blur-md z-40 flex items-center justify-between px-4">
        <div className="flex items-center">
          <button onClick={() => setIsMobileMenuOpen(true)} className="text-[#A1A1AA] p-1 mr-3">
            <Menu size={20} />
          </button>
      ...
      {/* Desktop Sidebar (Flex col, fixed height) */}
      <aside className="w-64 border-r border-[#262626] bg-[#000000] hidden md:flex flex-col h-full flex-shrink-0">
        ...
        <div className="p-6 border-t border-[#262626] flex items-center justify-between group cursor-pointer flex-shrink-0" onClick={handleLogout}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#171717] border border-[#262626] flex items-center justify-center">
        ...
        <div className="max-w-5xl mx-auto p-6 md:p-10 pt-20 md:pt-10">
              ...
              <AnimatePresence mode='popLayout'>
                {filteredNews.map((news) => (
                  <motion.div
                    ...
                    className="card p-5 cursor-pointer group flex flex-col h-full bg-[#0A0A0A] border border-[#262626] rounded-xl hover:border-[#3b82f6] transition-all overflow-hidden"
                  >
                    <div className="flex justify-between items-start mb-4 gap-2">
                      <span className="bg-[#171717] px-2.5 py-1 rounded-md text-[10px] font-bold text-white uppercase tracking-wider flex-shrink-0">
                        {news.subCat}
                      </span>
                      <span className="text-[#A1A1AA] text-[11px] truncate">{news.date}</span>
                    </div>
                    
                    <h3 className="text-base font-semibold text-white leading-snug mb-3 group-hover:text-[#3b82f6] transition-colors line-clamp-2">
                      {news.title}
                    </h3>
                    
                    <p className="text-sm text-[#A1A1AA] leading-relaxed mb-6 font-normal line-clamp-3">
                      {news.excerpt}
                    </p>
                    
                    <div className="mt-auto pt-4 border-t border-[#262626] flex justify-between items-center">
                      <span className="text-[10px] uppercase text-[#A1A1AA] font-bold block truncate max-w-[150px]">
                        Fonte: {news.source}
                      </span>
          ...
          {!isLoading && filteredNews.length === 0 && (
            <div className="py-20 text-center border border-dashed border-[#262626] rounded-xl mt-6">
              ...
              <p className="text-sm text-[#A1A1AA]">Ajuste seus filtros ou termo de busca.</p>
            </div>
          )}
        </div>
      </main>
